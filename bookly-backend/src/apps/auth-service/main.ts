...
async function bootstrap() {
  ...
  try {
    ...
    // CORS configuration
    const corsOrigins = configService.get<string[]>('auth.cors.origin', ['http://localhost:3000']);
    app.enableCors({
      origin: corsOrigins,
      credentials: true,
    ...
    const swaggerEnabled = configService.get<boolean>('auth.swagger.enabled', true);
    if (swaggerEnabled) {
      const config = new DocumentBuilder()
        .setTitle('Bookly Auth Service')
        .setDescription('Authentication and authorization service for Bookly reservation system')
        .setVersion('1.0.0')
        .addBearerAuth(
          {
            type: 'http',
