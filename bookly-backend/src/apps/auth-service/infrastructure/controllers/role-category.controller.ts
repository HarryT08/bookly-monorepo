...
import { SuccessResponseDto } from '@/libs/dto';
...
export class RoleCategoryController {
  ...
  async assignCategoriesToRole(
    ...
    return ResponseUtil.success(await this.commandBus.execute(command), 'Categories assigned to role successfully');
  }
  ...
  async removeCategoriesFromRole(
    ...
    const command = new RemoveCategoriesFromRoleCommand(roleId, dto, userEntity.id);
    await this.commandBus.execute(command);
    return ResponseUtil.success('Categories removed from role successfully');
  }
  ...
  async getRoleCategories(@Param('roleId') roleId: string) {
    const query = new GetRoleCategoriesQuery(roleId);
    return ResponseUtil.success(await this.queryBus.execute(query), 'Role categories retrieved successfully');
  }
}
