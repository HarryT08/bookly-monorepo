...
export class LoadBalancerService {
  ...
  constructor(
    ...
    this.strategy = this.configService.get<LoadBalancingStrategy>('gateway.loadBalancing.strategy', 'round-robin');
  ...
  private initializeServiceInstances(): void {
    ...
    this.logger.log(`Initialized load balancer with ${this.serviceInstances.size} services`);
  }
  ...
  public async getServiceUrl(serviceName: string): Promise<string> {
    ...
    if (healthyInstances.length === 0) {
      this.logger.warn(`No healthy instances for service: ${serviceName}, using unhealthy instances`);
  ...
  private async performHealthChecks(): Promise<void> {
    const healthCheckEnabled = this.configService.get<boolean>('gateway.loadBalancing.healthCheck.enabled', true);
    
    if (!healthCheckEnabled) {
      return;
    }

    const healthCheckTimeout = this.configService.get<number>('gateway.loadBalancing.healthCheck.timeout', 5000);
    const promises: Promise<void>[] = [];

    for (const [serviceName, instances] of this.serviceInstances.entries()) {
      for (const instance of instances) {
        promises.push(this.checkInstanceHealth(serviceName, instance, healthCheckTimeout));
      }
    }
  ...
  private async checkInstanceHealth(serviceName: string, instance: ServiceInstance, timeout: number): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(`Health check result for ${serviceName}: status=${response.status}, healthy=${instance.healthy}, responseTime=${responseTime}ms`);

      if (wasUnhealthy && instance.healthy) {
        this.logger.log(`Service instance ${instance.id} (${serviceName}) is now healthy`);
      }
  ...
  public addServiceInstance(serviceName: string, url: string, weight: number = 1): void {
    ...
    this.logger.log(`Added new instance ${instanceId} for service ${serviceName}: ${url}`);
  }
  ...
  public updateInstanceWeight(serviceName: string, instanceId: string, weight: number): boolean {
    const instances = this.serviceInstances.get(serviceName);
    ...
    this.logger.log(`Updated weight for instance ${instanceId} in service ${serviceName} to ${weight}`);
