...
async function bootstrap() {
  ...
  try {
    ...
    const host = configService.get<string>('availability.service.host', 'localhost');
    ...
    // CORS configuration
    const corsOrigins = configService.get<string[]>('availability.cors.origin', ['http://localhost:3000']);
    app.enableCors({
      origin: corsOrigins,
      credentials: true,
    ...
    const swaggerEnabled = configService.get<boolean>('availability.swagger.enabled', true);
    if (swaggerEnabled) {
      const config = new DocumentBuilder()
        .setTitle('Bookly Availability Service')
        .setDescription('Availability and reservation management service for Bookly reservation system')
        .setVersion('1.0.0')
        .addBearerAuth(
          {
            type: 'http',
