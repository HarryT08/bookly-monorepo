...
// Queries
import { GetAvailabilityQuery, GetResourceAvailabilityQuery, CheckAvailabilityQuery } from '../../application/queries/get-availability.query';
import { GetReservationHistoryQuery, ExportReservationHistoryQuery } from '../../application/queries/get-reservation-history.query';
...
export class AvailabilityController {
  ...
  @ApiQuery({ name: 'includeReservations', required: false, description: 'Include existing reservations' })
  @ApiQuery({ name: 'includeScheduleRestrictions', required: false, description: 'Include schedule restrictions' })
  ...
  async getCalendarAvailability(
    ...
    @Query('includeScheduleRestrictions') includeScheduleRestrictions: string = 'true'
  ) {
  ...
  @ApiResponse({ status: 400, description: 'Invalid input data' })
  async checkAvailability(@Body() availabilityQuery: AvailabilityQueryDto) {
  ...
  @ApiOperation({ 
    ...
    description: 'Creates a new reservation with comprehensive validation against availability and restrictions'
  })
  ...
  @ApiQuery({ name: 'reservationId', required: false, description: 'Filter by reservation ID' })
  ...
  async getReservationHistory(@Query() queryParams: ReservationHistoryQueryDto) {
  ...
  @ApiQuery({ name: 'resourceId', required: false, description: 'Filter by resource ID' })
  @ApiQuery({ name: 'provider', required: false, description: 'Filter by calendar provider' })
  ...
  @ApiResponse({ status: 200, description: 'Calendar synchronization completed successfully' })
  ...
  @ApiQuery({ name: 'resourceId', required: true, description: 'Resource ID to check availability for' })
  ...
  @ApiQuery({ name: 'includeConflicts', required: false, description: 'Include external calendar conflicts' })
  @ApiResponse({ status: 200, description: 'Availability with conflicts retrieved successfully' })
  ...
  @ApiQuery({ name: 'resourceId', required: false, description: 'Resource ID to show calendar for' })
  @ApiQuery({ name: 'startDate', required: true, description: 'Start date (ISO format)' })
  @ApiQuery({ name: 'endDate', required: true, description: 'End date (ISO format)' })
  @ApiQuery({ name: 'viewType', required: false, enum: CalendarViewType, description: 'Calendar view type' })
  @ApiQuery({ name: 'eventTypes', required: false, isArray: true, enum: EventType, description: 'Event types to include' })
  @ApiQuery({ name: 'includeAvailability', required: false, description: 'Include availability slots' })
  @ApiQuery({ name: 'includeExternalEvents', required: false, description: 'Include external calendar events' })
  @ApiQuery({ name: 'userId', required: false, description: 'User ID for personalized view' })
  ...
  @ApiQuery({ name: 'userId', required: false, description: 'Filter by user ID' })
  @ApiQuery({ name: 'resourceId', required: false, description: 'Filter by resource ID' })
  @ApiQuery({ name: 'actions', required: false, isArray: true, enum: HistoryAction, description: 'Filter by actions' })
  @ApiQuery({ name: 'sources', required: false, isArray: true, enum: HistorySource, description: 'Filter by sources' })
  ...
  @ApiQuery({ name: 'includeReservationData', required: false, description: 'Include reservation details' })
  @ApiQuery({ name: 'includeUserData', required: false, description: 'Include user information' })
  @ApiResponse({ status: 200, description: 'Detailed history retrieved successfully', type: ReservationHistoryResponseDto })
  ...
  async exportReservationHistory(
    ...
    res.setHeader('Content-Disposition', `attachment; filename="reservation-history-${new Date().toISOString().split('T')[0]}.csv"`);
    res.send(csvContent);
  }
