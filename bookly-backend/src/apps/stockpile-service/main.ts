...
async function bootstrap() {
  ...
  try {
    ...
    // CORS configuration
    const corsOrigins = configService.get<string[]>('stockpile.cors.origin', ['http://localhost:3000']);
    app.enableCors({
      origin: corsOrigins,
      credentials: true,
    ...
    const swaggerEnabled = configService.get<boolean>('stockpile.swagger.enabled', true);
    if (swaggerEnabled) {
      const config = new DocumentBuilder()
        .setTitle('Bookly Stockpile Service')
        .setDescription('Approval flows and validation service for Bookly reservation system')
        .setVersion('1.0.0')
        .addBearerAuth(
          {
            type: 'http',
