...
import { setTenant, setTenantColors, setTenantLoading, setTenantError } from '@/store/slices/tenantSlice';
import { generateTenantColors, applyTenantColors } from '@/utils/colors';
import { TenantConfig, DEFAULT_TENANT_CONFIG } from '@/types/tenant';

export const useTenant = () => {
  const dispatch = useAppDispatch();
  const { currentTenant, colors, isLoading, error } = useAppSelector((state) => state.tenant);
  ...
  const loadTenant = async (tenantId?: string) => {
    try {
      ...
      dispatch(setTenantError(err instanceof Error ? err.message : 'Failed to load tenant'));
    } finally {
   ...
   */
  const getTenantLogo = () => {
    return currentTenant.logo?.svg || currentTenant.logo?.url || DEFAULT_TENANT_CONFIG.logo?.svg;
  };
  ...
  useEffect(() => {
    ...
    // Extract tenant from subdomain (e.g., ufps.bookly.com -> ufps)
