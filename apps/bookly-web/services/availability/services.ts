...
export const availabilityService = {
	...
	async createAvailability(data: Omit<WeeklySchedule, 'id' | 'createdAt' | 'updatedAt'>): Promise<WeeklySchedule> {
		const response = await client.post(`${AVAILABILITY_API_BASE}/basic`, { json: data });
	...
	async getAvailability(params?: { resourceId?: string; dayOfWeek?: number }): Promise<WeeklySchedule[]> {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/basic?${searchParams}`);
		return response.json();
	},

	async updateAvailability(id: string, data: Partial<WeeklySchedule>): Promise<WeeklySchedule> {
		const response = await client.put(`${AVAILABILITY_API_BASE}/basic/${id}`, { json: data });
		return response.json();
	},

	async deleteAvailability(id: string): Promise<void> {
		await client.delete(`${AVAILABILITY_API_BASE}/basic/${id}`);
	},

	// Complex Schedule Operations
	async createSchedule(data: Omit<Schedule, 'id' | 'createdAt' | 'updatedAt'>): Promise<Schedule> {
		const response = await client.post(`${AVAILABILITY_API_BASE}/schedule`, { json: data });
		return response.json();
	},
	...
	async getSchedules(params?: { resourceId?: string; type?: string }): Promise<Schedule[]> {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/schedule?${searchParams}`);
		return response.json();
	},

	async updateSchedule(id: string, data: Partial<Schedule>): Promise<Schedule> {
		const response = await client.put(`${AVAILABILITY_API_BASE}/schedule/${id}`, { json: data });
		return response.json();
	},

	async deleteSchedule(id: string): Promise<void> {
		await client.delete(`${AVAILABILITY_API_BASE}/schedule/${id}`);
	},

	// Availability Check
	async checkAvailability(query: AvailabilityQuery): Promise<AvailabilityCheckResult> {
		const response = await client.post(`${AVAILABILITY_API_BASE}/check`, { json: query });
		return response.json();
	}
};
...
export const calendarIntegrationService = {
	async createIntegration(
		...
		const response = await client.post(`${AVAILABILITY_API_BASE}/calendar-integrations`, { json: data });
		return response.json();
	},
	...
	async getIntegrations(params?: {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/calendar-integrations?${searchParams}`);
		return response.json();
	},

	async updateIntegration(id: string, data: Partial<CalendarIntegration>): Promise<CalendarIntegration> {
		const response = await client.put(`${AVAILABILITY_API_BASE}/calendar-integrations/${id}`, { json: data });
		return response.json();
	},

	async deleteIntegration(id: string): Promise<void> {
		await client.delete(`${AVAILABILITY_API_BASE}/calendar-integrations/${id}`);
	},

	async syncIntegration(integrationId: string): Promise<{ success: boolean; eventsCount: number }> {
		const response = await client.post(`${AVAILABILITY_API_BASE}/calendar-integrations/${integrationId}/sync`);
		return response.json();
	},
	...
	async getAvailabilityWithConflicts(params: {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/availability-with-conflicts?${searchParams}`);
		return response.json();
	}
};
...
export const calendarViewService = {
	async getCalendarView(query: CalendarViewQuery): Promise<CalendarViewData> {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/calendar-view?${searchParams}`);
		return response.json();
	},
	...
	async getResourceCalendar(params: {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/calendar/${params.resourceId}?${searchParams}`);
		return response.json();
	}
};
...
export const reservationService = {
	async createReservation(data: CreateReservationRequest): Promise<Reservation> {
		const response = await client.post(`${AVAILABILITY_API_BASE}/reservations`, { json: data });
		return response.json();
	},
	...
	async getReservations(params?: {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/reservations?${searchParams}`);
		return response.json();
	},

	async getReservationById(id: string): Promise<Reservation> {
		const response = await client.get(`${AVAILABILITY_API_BASE}/reservations/${id}`);
		return response.json();
	},

	async updateReservation(data: UpdateReservationRequest): Promise<Reservation> {
		const response = await client.put(`${AVAILABILITY_API_BASE}/reservations/${data.id}`, { json: data });
		return response.json();
	},

	async cancelReservation(id: string, reason?: string): Promise<Reservation> {
		const response = await client.patch(`${AVAILABILITY_API_BASE}/reservations/${id}/cancel`, {
			json: { reason }
		});
		return response.json();
	},
	...
	async deleteReservation(id: string): Promise<void> {
		await client.delete(`${AVAILABILITY_API_BASE}/reservations/${id}`);
	}
};
...
export const reservationHistoryService = {
	async getHistory(query?: ReservationHistoryQuery): Promise<PaginatedResponse<ReservationHistory>> {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/reservation-history/detailed?${searchParams}`);
		return response.json();
	},

	async createHistoryEntry(data: Omit<ReservationHistory, 'id' | 'createdAt'>): Promise<ReservationHistory> {
		const response = await client.post(`${AVAILABILITY_API_BASE}/reservation-history`, { json: data });
		return response.json();
	},
	...
	async exportHistory(query?: ReservationHistoryQuery): Promise<Blob> {
		...
		const response = await client.get(`${AVAILABILITY_API_BASE}/reservation-history/export?${searchParams}`);
